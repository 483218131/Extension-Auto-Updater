...
- **🧹 路径自动复原**：采用位置堆栈技术，确保脚本运行结束后终端目录保持不变，对开发者极为友好。
- **🌙 智能后台防挂起**：完美识别无头后台任务模式，彻底解决计划任务无限期卡死（挂起）的顽疾。
- **🔑 Extension ID 永久锁定 (v1.1.0 新增)**：支持在配置中写入公钥，每次更新自动将其注入 `manifest.json`，彻底解决官方覆盖导致的本地配置与数据丢失问题。
...
> 2. 对于依赖浏览器原生同步的扩展，你可以提取扩展打包时生成的 `.pem` 公钥，将其写入 `config.json` 对应的 `key` 字段中。本工具会在每次更新后**自动为你注入** `manifest.json`，确保多台电脑上的 Extension ID 永远一致！
